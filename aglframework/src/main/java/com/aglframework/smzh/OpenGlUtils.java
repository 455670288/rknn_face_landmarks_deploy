...
public class OpenGlUtils {
    ...
    public static byte[] frameToBytes(IFilter.Frame frame) {
        ByteBuffer captureBuffer = ByteBuffer.allocate(frame.getTextureWidth() * frame.getTextureHeight() * 4);
        ...
        byte[] byteArray = new byte[captureBuffer.remaining()]; // 创建一个与 ByteBuffer 大小相同的 byte 数组
        captureBuffer.get(byteArray); // 读取 ByteBuffer 中的字节并存储到 byte 数组中
        return byteArray;
//        return captureBuffer.array();
    }

    public static Bitmap frameToBitmap(IFilter.Frame frame) {
        ByteBuffer captureBuffer = ByteBuffer.allocate(frame.getTextureWidth() * frame.getTextureHeight() * 4);
        captureBuffer.order(ByteOrder.nativeOrder());
        captureBuffer.rewind();
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, frame.getFrameBuffer());
