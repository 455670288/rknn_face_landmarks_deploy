...
//    public long post_time = 0;
    ...
    public int initModel(int im_height, int im_width, int im_channel, String modelPath,String modelPathFace) throws Exception {
        mOutputs = new OutputBuffer();
        mOutputs.mGrid0Out = new float[212];
        mOutputs.mGrid1Out = new float[212];
        ...
        if (navite_face_init(im_height, im_width, im_channel, modelPathFace) != 0) {
            throw new IOException("rknn face init fail!");
        }
        return 0;
    }
    ...
    public OutputBuffer run(byte[] inData) {
//        long startTime = System.currentTimeMillis();
//        long endTime;
//        Log.e("yyx","AN native_run --  native_run");
...
//
//            Recognition recog = new Recognition(mDetectResults.ids[i],
...
//        }
    ...
    private native int navite_face_init(int im_height, int im_width, int im_channel, String modelPathFace);
    private native void native_deinit();
    private native int native_run(byte[] inData, float[] grid0Out, float[] grid1Out, byte[] grid2Out);
    private native int native_post_process(byte[] grid0Out, byte[] grid1Out, byte[] grid2Out,
                                           int[] ids, float[] scores, float[] boxes);
    private native int native_inference();
